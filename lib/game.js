...
function Game(GameBase) {
    ...
    this.init = function(callback) {
        ...
        this.backendCommunicator.eventEmitter.on('match.terminate', function() {
            process.exit();
        });
    ...
    this.finish = function() {
        ...
        for (let slot of self.GameBase.Players.getSlots()) {
            if (slot.getPoints() === maxPoints && slot.getUser() !== null) {
    ...
    this.setProgress = function(progress) {
        if (progress < 0 || progress > 1) throw new Error('GameBase.Game.setProgress: progress must be between 0 and 1');
    ...
    this.setStatusMessage = function(text) {
        this.backendCommunicator.sendCommand('event', {
