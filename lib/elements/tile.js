...
function Tile() {
    ...
    this.on('tile.move', async (slotIndex, data) => {
        this.moveTo(
        ...
        if (typeof this.onAfterMove === 'function') {
            await this.onAfterMove.call(this, slotIndex, this.getParent(), this);
        }
    });

    this.on('tile.flip', async slotIndex => {
        this.flip();
    ...
    this.on('tile.rotate', async (slotIndex, data) => {
