...
function PieceContainer() {
    ...
    this.calculateNextIndexes = function(startIndex, steps, slotIndex) {
        if (steps > 0) {
            let indexes = [];
            for (let i = 0; i < positions.length; i++) {
                if (positions[i].index === startIndex && typeof positions[i].next[slotIndex] !== 'undefined') {
                    for (let j = 0; j < positions[i].next[slotIndex].length; j++) {
                        indexes = indexes.concat(this.calculateNextIndexes(positions[i].next[slotIndex][j], steps - 1, slotIndex));
                    }
                }
            }
            return indexes;
